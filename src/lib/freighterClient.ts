...
/** Network Credence operates against: Stellar mainnet (`public`) or testnet (`test`). */
...
/**
 * Maps a Freighter network identifier (e.g. `PUBLIC`, `TESTNET`, `MAINNET`) to a
 * Credence network value. Matching is case-insensitive and substring-based.
 *
 * @param freighterNetwork - Network string reported by Freighter.
 * @returns `'test'`, `'public'`, or `null` when the value is unrecognized.
 */
